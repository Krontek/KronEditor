...
func (s *Server) logAIChat(req aiChatReq, projectContext string, msg aiMessage, callErr error, took time.Duration) {
	...
	// ⚠️ Count req.Context, NOT the saved projectContext copy: for every
	// non-Anthropic provider handleAIChat has already folded the context INTO
	// req.System and cleared req.Context, so adding the saved copy would report
	// the project context twice — and overstate the prompt exactly for the
	// providers that get no cache discount, which is where the number matters.
	sysChars := len(req.System) + len(req.Context)
	histChars := 0
	for _, m := range req.Messages {
		...
		// Image attachments are base64 and routinely dwarf the text; a size line
		// that omitted them would understate the very turns that are slowest.
		for _, img := range m.Images {
			histChars += len(img.Data)
		}
	}
