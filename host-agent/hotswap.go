...
func hostLinkRecipe() string {
	...
	case "darwin":
		// Mirrors compileHost's darwin case: -export_dynamic, and NO -ldl/-lrt
		// (macOS ships neither; both live in libSystem).
		return "darwin:export_dynamic+pthread"
	default:
...
func (s *Server) handleHotSwapRun(w http.ResponseWriter, r *http.Request) {
	...
	if perr != nil {
		// ⚠️ Read the process state BEFORE Stop(). A host that died on its own
		// says WHY through its exit status — the only signal left when it never
		// reached main (Windows STATUS_DLL_NOT_FOUND being the classic case) —
		// but Stop() KILLS the process and then waits on `done`, so afterwards
		// `done` is closed either way and the two cases become
		// indistinguishable. Sampling first keeps the genuine-timeout path
		// (host alive but slow, killed by us) reporting the timeout instead of
		// a bogus "exited (status 0xFFFFFFFF)" from our own SIGKILL.
		diedOnItsOwn := exited(done)
		var exitState *os.ProcessState
		if diedOnItsOwn {
			// Safe: the reaper's cmd.Wait has returned iff done is closed.
		...
		if exitState != nil {
			msg = fmt.Sprintf("loader-host exited (%s) without reporting a cold-start result", describeExit(exitState))
			if uint32(exitState.ExitCode()) == 0xC0000135 {
...
// describeExit renders a finished process's outcome for a user-facing error.
//
// Windows exit codes for a process that died before main are NTSTATUS values
// (0xC0000135 = STATUS_DLL_NOT_FOUND), which only read as anything in hex. On
// Unix a signal death gives ExitCode() == -1, so the hex form would print a
// meaningless 0xFFFFFFFF and hide the signal — ProcessState.String() says
// "signal: killed" / "exit status 1" instead.
func describeExit(st *os.ProcessState) string {
	if runtime.GOOS == "windows" {
		return fmt.Sprintf("status 0x%08X", uint32(st.ExitCode()))
	}
	return st.String()
}
