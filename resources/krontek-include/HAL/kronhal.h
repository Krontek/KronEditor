...
static inline int32_t KRON_USB_PortBaud(uint8_t port) {
    (void)port;
    return 115200;
}
#endif
...
/* Servo/ESC software-PWM outputs are NOT a ladder block — they are a board
 * runtime service (KRON_SERVO_* at the end of this file), configured in the
 * board interface config and wired by the transpiler. The PLC scan writes a
 * plain REAL variable (pulse width in µs); a background thread produces the
 * waveform. See the KRON_SERVO_* section below. */
...
/* ===================================================================
 * USB Serial Send  (channel-based, max 5)
 * =================================================================*/
typedef struct {
    uint8_t DATA;
    int32_t BAUD;
...
typedef HAL_USB_Send USB0_Send;
typedef HAL_USB_Send USB1_Send;
typedef HAL_USB_Send USB2_Send;
typedef HAL_USB_Send USB3_Send;
typedef HAL_USB_Send USB4_Send;

/* ===================================================================
 * USB Serial Receive  (channel-based, max 5)
 * =================================================================*/
typedef struct {
    int32_t BAUD;
    int16_t TIMEOUT;
...
typedef HAL_USB_Receive USB0_Receive;
typedef HAL_USB_Receive USB1_Receive;
typedef HAL_USB_Receive USB2_Receive;
typedef HAL_USB_Receive USB3_Receive;
typedef HAL_USB_Receive USB4_Receive;
...
/* USB Serial Send */
static inline void USB0_Send_Call(HAL_USB_Send *i) { HAL_USB_Send_Call(i, 0); }
static inline void USB1_Send_Call(HAL_USB_Send *i) { HAL_USB_Send_Call(i, 1); }
static inline void USB2_Send_Call(HAL_USB_Send *i) { HAL_USB_Send_Call(i, 2); }
static inline void USB3_Send_Call(HAL_USB_Send *i) { HAL_USB_Send_Call(i, 3); }
static inline void USB4_Send_Call(HAL_USB_Send *i) { HAL_USB_Send_Call(i, 4); }

/* USB Serial Receive */
static inline void USB0_Receive_Call(HAL_USB_Receive *i) { HAL_USB_Receive_Call(i, 0); }
static inline void USB1_Receive_Call(HAL_USB_Receive *i) { HAL_USB_Receive_Call(i, 1); }
static inline void USB2_Receive_Call(HAL_USB_Receive *i) { HAL_USB_Receive_Call(i, 2); }
static inline void USB3_Receive_Call(HAL_USB_Receive *i) { HAL_USB_Receive_Call(i, 3); }
static inline void USB4_Receive_Call(HAL_USB_Receive *i) { HAL_USB_Receive_Call(i, 4); }
...
/* Generic USB_Send — delegates to channel-based HAL_USB_Send_Call */
static inline void USB_Send_Call(USB_Send *inst) {
    const bool rising = inst->Execute && !inst->_prev_execute;
    inst->Done = false;
    if (inst->_phase == KRON_COMM_PHASE_IDLE && rising) {
        inst->Busy = true;
        ...
        const uint8_t *buffer = (const uint8_t *)inst->pTxBuffer;
        inst->_err_id = 0;
        if (inst->Length == 0U || buffer == NULL) {
            ok = false;
            inst->_err_id = 1;
        }
        const int32_t baud = KRON_USB_PortBaud(inst->Port_ID);
        for (index = 0; ok && index < inst->Length; index++) {
            HAL_USB_Send tx = {0};
            tx.EN = true;
            tx.BAUD = baud;
            tx.DATA = buffer[index];
            HAL_USB_Send_Call(&tx, inst->Port_ID);
            ok = tx.DONE && tx.ERR_ID == 0;
            inst->_err_id = tx.ERR_ID;
        }
        inst->Done = ok;
...
/* Generic USB_Receive — delegates to channel-based HAL_USB_Receive_Call */
static inline void USB_Receive_Call(USB_Receive *inst) {
    inst->NewData = false;
    inst->ReceivedLength = 0;
    inst->Error = false;
    inst->_err_id = 0;
    if (!inst->Enable) return;
    uint8_t *buffer = (uint8_t *)inst->pRxBuffer;
    if (buffer == NULL || inst->MaxSize == 0U) {
        inst->Error = true;
        inst->_err_id = 1;
        return;
    }
    const int32_t baud = KRON_USB_PortBaud(inst->Port_ID);
    uint16_t count = 0;
    for (uint16_t i = 0; i < inst->MaxSize; i++) {
        HAL_USB_Receive rx = {0};
        rx.EN = true;
        rx.BAUD = baud;
        rx.TIMEOUT = 0;
        HAL_USB_Receive_Call(&rx, inst->Port_ID);
        if (rx.ERR_ID != 0) { inst->_err_id = rx.ERR_ID; inst->Error = true; break; }
        if (!rx.READY) break;
        buffer[i] = rx.DATA;
        count++;
    }
    inst->ReceivedLength = count;
    inst->NewData = count > 0U;
}
...
/* ===================================================================
 * Servo / ESC software-PWM runtime  (board service, not a ladder block)
 *
 * Bit-bangs a servo/ESC pulse train (default 50 Hz) on one or more GPIO
 * pins with sub-µs HIGH-width accuracy. The waveform is produced by ONE
 * SCHED_FIFO thread that schedules each frame with
 * clock_nanosleep(TIMER_ABSTIME) and busy-waits the exact HIGH microseconds
 * per channel — the same technique as a standalone RT ESC driver. The PLC
 * scan NEVER toggles the pin: it writes a plain REAL setpoint variable (the
 * pulse width in µs), and the thread reads that variable's address each
 * frame. So scan jitter can never reach the waveform. Toggling goes through
 * GPIO_Write_Call → board-agnostic (works on any HAL that implements it).
 *
 * Wiring (emitted by the transpiler from the board interface config):
 *   KRON_SERVO_Register(idx, pin, period_ns, &S->setpoint_us);   // per channel
 *   KRON_SERVO_RuntimeInit();                                    // in PLC_Init
 *   KRON_SERVO_RuntimeCleanup();                                 // in PLC_Cleanup
 *
 * A setpoint in [1000, 2000] µs is pulsed; a setpoint BELOW 1000 (e.g. 0, or
 * NaN) CUTS the output — no pulse at all, the line is held low. So the scan
 * writes 0 to stop the signal entirely at neutral/zero instead of holding a
 * 1500 µs neutral pulse. A value above 2000 is clamped to 2000. (During ESC
 * arming the scan must still write ~1500 so the ESC sees neutral pulses.)
 *
 * NOTE: the driver thread + its GPIO fds are NOT hot-swap trampolined (like
 * EtherCAT/motion) — deploy with Build & Send, not "Go Live".
 * =================================================================*/
#if defined(__linux__) && !defined(HAL_SIM_MODE)
#include <sched.h>
#include <sys/mman.h>
#include <time.h>

#ifndef KRON_SERVO_MAX_CH
#define KRON_SERVO_MAX_CH 8
#endif
#define KRON_SERVO_NEUTRAL_US 1500.0f
#define KRON_SERVO_MIN_US     1000.0f
#define KRON_SERVO_MAX_US     2000.0f

typedef struct {
    int             pin;        /* physical header pin this channel drives */
    long            period_ns;  /* frame period for this channel */
    volatile float *us;         /* PLC-owned setpoint (µs); read every frame */
} KronServoChannel;

static KronServoChannel __kron_servo_ch[KRON_SERVO_MAX_CH];
static int              __kron_servo_count = 0;
static long             __kron_servo_period_ns = 20000000L;  /* min of registered periods */
static pthread_t        __kron_servo_thread;
static volatile bool    __kron_servo_running = false;

static inline long __kron_servo_now_ns(void) {
    struct timespec t; clock_gettime(CLOCK_MONOTONIC, &t);
    return (long)t.tv_sec * 1000000000L + t.tv_nsec;
}

/* Toggle a pin through the board GPIO HAL. */
static inline void __kron_servo_set(int pin, int value) {
    GPIO_Write w; memset(&w, 0, sizeof(w));
    w.PIN = (int16_t)pin; w.EN = true; w.VALUE = value ? true : false;
    GPIO_Write_Call(&w);
}

static void *__kron_servo_thread_fn(void *arg) {
    (void)arg;
    /* Real-time posture: max FIFO priority + locked memory. CPU pinning is a
     * refinement, only compiled when built with _GNU_SOURCE (CPU_SET). */
...
#ifdef CPU_SET
    {
        ...
        if (ncpu > 1) {
            cpu_set_t cs; CPU_ZERO(&cs); CPU_SET((int)(ncpu - 1), &cs);
    ...
    const int  n = __kron_servo_count;         /* fixed after RuntimeInit */
    struct timespec next; clock_gettime(CLOCK_MONOTONIC, &next);
    while (__kron_servo_running) {
        long w_ns[KRON_SERVO_MAX_CH];
        bool hi[KRON_SERVO_MAX_CH];
        int  remaining = 0;
        /* read each setpoint and raise its line HIGH — UNLESS the setpoint is
         * below the minimum (or NaN): then CUT the output entirely (no pulse,
         * line held low). The scan writes 0 to stop the signal at neutral/zero
         * instead of holding a neutral 1500 µs pulse. */
        for (int i = 0; i < n; i++) {
            float us = *__kron_servo_ch[i].us;
            if (!(us >= KRON_SERVO_MIN_US)) {          /* < MIN or NaN → cut */
                __kron_servo_set(__kron_servo_ch[i].pin, 0);
                hi[i] = false;
                continue;
            }
            if (us > KRON_SERVO_MAX_US) us = KRON_SERVO_MAX_US;
            w_ns[i] = (long)(us * 1000.0f);
            __kron_servo_set(__kron_servo_ch[i].pin, 1);
            hi[i] = true; remaining++;
        }
        long t0 = __kron_servo_now_ns();
        /* busy-wait, dropping each line as its HIGH width elapses (sub-µs) */
        while (remaining > 0 && __kron_servo_running) {
            long el = __kron_servo_now_ns() - t0;
            for (int i = 0; i < n; i++) {
                if (hi[i] && el >= w_ns[i]) {
                    __kron_servo_set(__kron_servo_ch[i].pin, 0);
                    hi[i] = false; remaining--;
                }
            }
        }
        /* absolute-time next frame → no drift */
        next.tv_nsec += __kron_servo_period_ns;
        while (next.tv_nsec >= 1000000000L) { next.tv_nsec -= 1000000000L; next.tv_sec++; }
        clock_nanosleep(CLOCK_MONOTONIC, TIMER_ABSTIME, &next, NULL);
    }
    for (int i = 0; i < n; i++) __kron_servo_set(__kron_servo_ch[i].pin, 0);
    return NULL;
}

static inline void KRON_SERVO_Register(int idx, int pin, long period_ns, volatile float *us_ptr) {
    if (idx < 0 || idx >= KRON_SERVO_MAX_CH || us_ptr == NULL) return;
    __kron_servo_ch[idx].pin       = pin;
    __kron_servo_ch[idx].period_ns = period_ns;
    __kron_servo_ch[idx].us        = us_ptr;
    if (idx + 1 > __kron_servo_count) __kron_servo_count = idx + 1;
    if (period_ns > 0 && period_ns < __kron_servo_period_ns) __kron_servo_period_ns = period_ns;
}

static inline void KRON_SERVO_RuntimeInit(void) {
    if (__kron_servo_count == 0 || __kron_servo_running) return;
    for (int i = 0; i < __kron_servo_count; i++) __kron_servo_set(__kron_servo_ch[i].pin, 0);
    __kron_servo_running = true;
    if (pthread_create(&__kron_servo_thread, NULL, __kron_servo_thread_fn, NULL) != 0)
        __kron_servo_running = false;
}

static inline void KRON_SERVO_RuntimeCleanup(void) {
    if (!__kron_servo_running) return;
    __kron_servo_running = false;
    pthread_join(__kron_servo_thread, NULL);
    for (int i = 0; i < __kron_servo_count; i++) __kron_servo_set(__kron_servo_ch[i].pin, 0);
}
#else
/* Non-Linux / simulation: no real waveform. Stubs keep generated code building
 * and simulatable (the setpoint variable still updates in the sim). */
static inline void KRON_SERVO_Register(int idx, int pin, long period_ns, volatile float *us_ptr) {
    (void)idx; (void)pin; (void)period_ns; (void)us_ptr;
}
static inline void KRON_SERVO_RuntimeInit(void) {}
static inline void KRON_SERVO_RuntimeCleanup(void) {}
#endif
