...
export const transpileToC = (projectStructure, standardHeaders = [], boardId = null, simMode = true, buses = [], busConfigs = {}) => {
    ...
    const ecCfgEarly = generateEtherCATConfig(buses, busConfigs);
    ...
    if (projectStructure.taskConfig?.tasks?.length > 0) {
        projectStructure.taskConfig.tasks.forEach(task => {
            ...
            const progs = [...(task.programs || [])]
                ...
                .filter(Boolean);
            if (progs.length > 0) {
                taskGroups.push({ taskName: (task.name || task.id).replace(/\s+/g, '_'), intervalUs, programs: progs });
            }
            progs.forEach(pName => {
    ...
    if (pou.type === 'ST') {
        ...
        (pou.content.rungs || []).forEach(rung => {
            if (rung.lang === 'ST') {
                ...
                src += `    // SCL rung [LD]\n`;
                src += transpileLDLogics([rung], stdFunctions, safeName, category, globalVarNames, inputShadowMap);
            }
        });
    }
...
const FB_INPUT_TYPES = {
    ...
    // Motion control: only special types listed; BOOL/REAL use defaults
    'MC_Power': { 'Axis': 'AXIS_REF' },
    'MC_Home': { 'Axis': 'AXIS_REF' },
    'MC_Stop': { 'Axis': 'AXIS_REF' },
    'MC_Halt': { 'Axis': 'AXIS_REF' },
    'MC_MoveAbsolute': { 'Axis': 'AXIS_REF' },
    'MC_MoveRelative': { 'Axis': 'AXIS_REF' },
    'MC_MoveAdditive': { 'Axis': 'AXIS_REF' },
    'MC_MoveVelocity': { 'Axis': 'AXIS_REF' },
    'MC_MoveContinuousAbsolute': { 'Axis': 'AXIS_REF' },
    'MC_MoveContinuousRelative': { 'Axis': 'AXIS_REF' },
    'MC_SetPosition': { 'Axis': 'AXIS_REF' },
    'MC_SetOverride': { 'Axis': 'AXIS_REF' },
    'MC_Reset': { 'Axis': 'AXIS_REF' },
    'MC_ReadActualPosition': { 'Axis': 'AXIS_REF' },
    'MC_ReadActualVelocity': { 'Axis': 'AXIS_REF' },
    'MC_ReadActualTorque': { 'Axis': 'AXIS_REF' },
    'MC_ReadStatus': { 'Axis': 'AXIS_REF' },
    'MC_ReadMotionState': { 'Axis': 'AXIS_REF' },
    'MC_ReadAxisInfo': { 'Axis': 'AXIS_REF' },
    'MC_ReadAxisError': { 'Axis': 'AXIS_REF' },
};
