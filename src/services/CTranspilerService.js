...
`;
    ...
    if (shmEntries.length > 0) {
        ...
        // In a hot-swap build the loader-host owns the /dev/shm mirror (so the
        // editor keeps reading live variables across a logic swap) and assigns
        // __plc_shm; the logic .so just references it (extern), like us_tick.
...
const generateMainLoop = (projectStructure, config, boardId = null, shmEnabled = false, execTimeVars = [], initCode = '', cleanupCode = '', ecPdoReadCode = '', ecPdoWriteCode = '', ecThreadCode = '', ecThreadStartCode = '', ecThreadJoinCode = '', gpiMutexEnabled = false, shmEntries = []) => {
    ...
    // logic.so. The generic loader-host dlopen's it and drives the scan via
