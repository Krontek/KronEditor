...
**`deploy_server_to_target`** (`deploy_ssh.go`): SFTP the prebuilt `plc-agent` to `<home>/plc/plc-agent`, install a supervisor (systemd unit, else a restart-on-crash script), start, poll `:7070/status`.
- ⚠️ **Never shell out to `pkill`/`crontab`** — minimal images ship neither, so `pkill -f … 2>/dev/null` was a SILENT no-op leaving one supervisor per deploy. `killPatternPrologue` glob-scans `/proc/*/cmdline` and ⚠️ **skips its own PPID chain**: every deploy command line contains the pattern it kills (a sentinel string protects only callers that carry it).
- ⚠️ **Boot persistence is BEST-EFFORT and every hook must be PROBED, not assumed** — `probeBootCaps` asks the target which hooks it actually *executes*, then `installBootHook` installs all of them (init.d `S99plc-agent` + the **`initdefault` runlevel** symlink, `@reboot` cron, `rc.local`) and **read-back-verifies each**, else `none` + a warning; never fail a deploy over it. Two silent no-ops came from assuming: BusyBox/Yocto init never reads `/etc/rc.local` (it runs `rcS` → `rc<N>.d/S*`), and BusyBox `sed` rejects the GNU `0,/re/` first-match address — both reported success. Verified by reboot on rpi3bplus. SSE topic `server-deploy-progress`.
...
- ⚠️ **`serverBinaryForBoard` maps board-id prefix → server arch** (aarch64 → `_arm64`, `bb_` → `_armv7`, else amd64); an unlisted aarch64 family ships the wrong arch.
...
- ⚠️ **Locate the bundled GCC dir by GLOB, not from the compile triple** (`paths.go LLVMGCCInstallDir`, used as `-B`/`-L`): sysroots name it with a vendor clang does not auto-detect (`arm-none-…`, `x86_64-buildroot-…`; aarch64 matches by luck), so armv7 **and x86_64** `-static` builds died with `cannot open crtbeginT.o` — all three Linux targets pass `-B`, and `setup_toolchain.py` harvests `lib/gcc` into each of their sysroots. Version dirs use `compareVersionStrings` (numeric).
...
⚠️ **`KRON_FN` is the LD path ONLY** — in ST text `MIN/MAX/LIMIT/SEL/MUX` reached C verbatim (clang: `unknown type name`, the phantom-block symptom on valid IEC). `expandSTSelectionCalls` emits plain C conditionals (positional + named args, nested, loud arity errors), deliberately NOT `KRON_LIMIT`/`KRON_MIN`: those `_Generic`-dispatch on ONE argument (`mn`, `in0`), so a literal bound picks the int32 overload and silently truncates a REAL — ⚠️ **the LD path still has that hazard**. Its shadow guard covers user FUNCTION/FB names, never variables (it matches a call form; including them broke `MIN(a,b)` in a POU declaring `Min`). ST has no `SQRT`/`EXPT`.
...
⚠️ **The task interval bounds every timer:** a self-resetting TON needs 2 extra scans per half-cycle, so the observed half-period is `PT + 2 × task_interval` (PT=1s on a `T#1s` task toggles every 3.00 s). Keep the interval an order of magnitude under the shortest measured time.
...
⚠️ **exec-time is MICROseconds** (`__exec_us_<prog>`); changing a unit means grepping the FORMATTERS too — `fmtExecNs` survived the ns→µs migration.

**Gates:** `experiments/transpiler-check/compile-gate.sh` (**run after transpiler changes**) and `experiments/agent-check/gate.sh` (§8) — ⚠️ both are gitignored, absent from a fresh clone; without them, bundle the transpiler with `esbuild --loader:.tpl=text` and compile its output with `toolchains/bin/clang -fsyntax-only`.
...
- ⚠️ **Timing primitives lie by default:** winpthreads' `clock_nanosleep(ABSTIME)` doesn't block (a 10 ms task ran ~6 M times/s), macOS has none and `mach_timebase_info` isn't 1/1 on Apple Silicon; both derive a relative delta from one absolute deadline in **64-bit** (mingw's `tv_nsec` is 32-bit → an interval ≥ ~2.148 s went negative).
...
- ⚠️ **Every cold-start failure path in host.c's `main` must `write_swap_result` before returning** — that file is the supervisor's only channel. Sample `exited(done)`/`ProcessState` **before** `Stop()` (kills then waits), else a real timeout reads as `exited (signal: killed)`.
...
**Re-attach after a browser reload:** `GET /api/host/sim-status` → `{running, pid, mode}`; an effect guarded by `simReattachedRef` restores the flags plus `hotSwapActiveRef`/`taskSigRef`. ⚠️ For a **remote** PLC the 3 s `checkStatus` poll starts the SSE **unconditionally** — `remoteVarKeysRef` is set only by Build & Send in the same session.
...
**Rung geometry ⚠️:** `MIDDLE_Y` is **FIXED at `MIN_RUNG_HEIGHT/2`** — never proportional to the dynamic `RUNG_HEIGHT`, or the rail dots move as a rung grows. The `[10,10]` grid can't land a contact's handle on `MIDDLE_Y`, so `powerHandleOffsetY(nodeId)` measures it from the DOM and drag/drop snap to `MIDDLE_Y - offset`. ⚠️ **Agent-authored blocks never take the drop path**, so they inline the same arithmetic (`LD_*` in `agentTools.js`) — **if `RungContainer` geometry changes, these follow**; `insertBlock`/`addBlockToRung` **return the block id** for the snap.
...
- ⚠️ **Truncation is the other dead-turn source:** `defaultMaxTokens` is **32000** (thinking tokens bill against it); a max_tokens/length finish **discards the turn and fails loudly** — a tool call cut mid-arguments comes back partially parsed and looks well-formed.
...
- ⚠️ **Hardware blocks reach the agent ONLY through `args.__hardware`** (`getBoardBlockDefs(board, ifaceConfig)` — the shared source the Toolbox and VariableManager also use): no XML declares GPIO/PWM/ADC/CAN, and the transpiler registers them for the duration of ONE transpile, so a catalog built from `libraryData` alone made the agent insist the board has no I/O at all. They are EN/ENO blocks — `powerPins(…, isHardware)` mirrors `halBlockMeta`, since the generic first-BOOL fallback picks `GPIO_Write.VALUE` — and the board's list rides `<project-context>`, never the cached system prompt. ⚠️ **A power-wired one must carry `data.executionControl: true`**: RungContainer strips EN/ENO from a board block's pin body, so without the flag those handles don't exist, ReactFlow drops both edges and the rung LOOKS like an unconnected island (codegen was correct all along); the flag also makes `LD_FB_Y`'s "trigger is the first pin row" true.
- ⚠️ **No compile/build tool, deliberately** — `check_compile` made every edit pay clang's cold load.
- ⚠️ **Local file access is permission-gated and OFF by default** (📁 button → one root folder, read or read+write): `FS_TOOL_DEFS`/`FS_WRITE_TOOL_DEFS` stay OUT of `TOOL_DEFS` and are appended in `runTurn` only while granted — a model that can SEE a tool it may not use retries it and blames the project. `resolveInRoot` (segment-prefix containment) is the whole guard; the host agent's file endpoints don't sandbox.
...
⚠️ **Device-node overrides (UART+I2C+USB) reach the HAL** via `buildRuntimePortHelpers`, emitting `#define KRON_UART<n>/KRON_I2C<n>/KRON_USB<n>` from `content.deviceInterfaceConfig` *before* `kron_hal.h`; SPI has none.
...
- ⚠️ **PWM channels are FLATTENED ACROSS pwmchips**, never "the first chip with enough channels" — generic SBCs sharing `kronhal_rpi.h` publish one pwmchip **per PWM cell**, so the old probe matched nothing and PWM was dead on all 12. Chips are collected, **sorted by index** (readdir isn't) and their `npwm` concatenated.
...
- ⚠️ **A stub-bodied block carries a `note` in `BLOCK_NOTES` (`boardLibraryBlocks.js`) → `warning` in the agent's catalog.** `PCM_*` (all families) and `Grove_*` (bb_green) are empty stubs and `PCM_Output` even sets `OK = EN`, so a human at least sees an unfinished block while the agent had no cue at all. Delete an entry when its `_Call` lands.
...
- ⚠️ **`Server.Bind()` claims `:7070` BEFORE `pm.CleanupOrphanRuntime()`; `Start()` then serves.** A surplus agent must lose the port before touching shared state — two supervisors respawning one every 2 s each reaped the *live* runtime as an "orphan" (`crashed / exit code -1` after ~2 s, no stderr). Serving still begins after the reaper, so a `Start` RPC is never mistaken for one.
- ⚠️ **`/dev/shm` may not exist** (BusyBox/Yocto never create it): `ensureShmDir` mkdirs 1777 + best-effort tmpfs mount, else `O_CREAT` gives ENOENT, the agent `exit(1)`s in a 2 s crash-loop, and the runtime's `shm_open` fails the same way.
...
## 13. EtherCAT & motion
**Motion:** the `MOTION_FB_AXIS_PARAM` set — every `MC_*` block calls `MC_xxx_Call(&inst, &axisVar)`; `Axis` (AXIS_REF) is not a struct field (skipped in value assignment, null-init and shadow vars). All `motion.xml` blocks list `Axis` first.
- ⚠️ **`KronMotion` was rewritten around an external trajectory core (2026-09):** `kron_nc.c/.h` are GONE, `AXIS_REF` moved to `kron_axis.h`, every numeric pin is a **`double` (LREAL, never REAL)**, and `kron_axis.h` includes `TrajectoryGenerator.h` from `github.com/fehimkus/OnlineTrajectoryGenerator` — fetched by `externalLibDeps` (§15), not by KronMotion's CMake.
- ⚠️ **The axis engine runs in the LOGIC task, not the IO_Bus thread** (`generateMainLoop`'s `axisReadPhase`/`axisWritePhase` wrap `plc_task_body_<i>`): `KronAxis_Submit` hands a request POINTER inside the FB instance to `AXIS_REF.Active/Buffer[]` with no lock — nothing replaces the old `cmd_Seq`/`sts_AckSeq` handshake that let the NC engine live in another thread. The owning task is the one whose programs DECLARE an `MC_*` instance (>1 such task ⇒ warn); `dt` is that task's interval, not the fieldbus cycle.
- ⚠️ **plc.c must NOT define `Kron_PI` / `Kron_HAL`** — `kron_pi.c` (→ `libkron_pi.a`) owns both, so defining them again is `multiple definition` at link. `Kron_HAL` stays NULL: the generated code does the PDO exchange itself and calls the PER-AXIS `KronAxis_ReadInput`/`WriteOutput`, never `KronMotion_ReadInputs/WriteOutputs` (those go through the HAL pointer).
- ⚠️ **`kronmotion.h` is the source of truth for SIX places** — `FB_INPUTS`+`FB_OUTPUTS` (together they must equal the struct's non-`_` fields), `FB_TRIGGER_PIN`/`FB_Q_OUTPUT`, `MOTION_FB_AXIS_PARAM`, `motion.xml`, and `libraryTree.js` (omitted → never in the Toolbox). A pin named in a table but absent from the struct is a **compile** error, the reverse is a silently dead pin.
- ⚠️ **`MOTION_FB_OUTPUT_TYPES` is mandatory for every non-BOOL MC_ output** — `getOutputPinType` keys off pin NAMES and falls through to BOOL, so `Position`/`Velocity`/`Torque`/`Value`/`CoveredDistance` (LREAL) and `ErrorID`/`AxisErrorID` (WORD) collapse to one bit without it.
- ⚠️ **`AXIS_REF_DEBUG_FIELDS` (transpiler) and `AXIS_REF_BUILTIN` (VariableManager) hand-mirror the struct**; offsets are x86_64 `offsetof` for the sim's `/proc/mem` reader, and a name IS the C member path (`Set.Position`). Re-verify with a probe after any AXIS_REF edit.
**Config generation:** `generateEtherCATConfig(buses, busConfigs)` → `KRON_EC_Config` init C; `kron_ec_init`/`kron_ec_close` in PLC_Init/Cleanup, and per task `kron_ec_pdo_read` before `plc_shm_pull`, `kron_ec_pdo_write` after `plc_shm_sync`. Files: `EtherCATEditor.jsx` + `deviceCodegen.js`, `SlaveConfigPage.jsx` + `EsiLibraryService.js`.
...
**Blocks are declared in `public/libraries/*.xml`** (`<block type>` + `<inputs>`/`<outputs>` `<pin name type [trigger]>`, grouped in `<category>`), arranged for the Toolbox by `libraryTree.js`.
...
**Implementing one: `kronsystem.h` is the pattern** — other blocks live in a prebuilt `libkron*.a` and §1 forbids generating `.a`s, so this header holds `static inline` bodies compiled into `plc.c`/`logic.so`. ⚠️ It is the ONE file under `krontek-include/` besides `HAL/` that is **git-tracked**. It carries **27 blocks**: RTC/calendar, scheduling, diagnostics, Linux-only host health (temp/load/disk, else `ERR_ID=1`), TIME arithmetic, signal shaping, and the sysfs readers `Read_Sys_File` / `Read_Hwmon` (which finds a device **by `name`**, since the `hwmonN` index is probe-order and moves across boots). ⚠️ **The readers only READ** — they publish the file's text on a STRING `VALUE` and leave meaning to the program (compare, or convert); parsing inside them would force a number onto files that hold a word. **Five** things must line up — the XML and a `libraryTree.js` entry (else it never reaches the Toolbox), plus:
...
⚠️ **STRING pins — quoting is OPTIONAL and a bare word is TEXT.** `STRING` maps to `char *` and gets **no writable input shadow var** — the literal is re-assigned every scan, which is also what keeps it valid across a hot-swap. Quoted `'…'`/`"…"` is always text; a bare word resolves to a variable **only if one is declared**, else it is text too — so a path needs no quotes and `NAME := pwmfan` means the word, not an invented variable. ⚠️ **Quotes are DELIMITERS everywhere, including `initialValue`** (`formatVarInitial` once emitted `"'/sys/…/'"`, unopenable) — one decode/escape pair owns this (`unquoteIecString`/`iecStringToC`), and `collectUndeclaredPinVars` plus the agent's `set_ladder` hint collector must both SKIP string pins or they mint a BOOL/empty STRING behind the user's back. ⚠️ **STRING IS a live value, read-only, via a fixed 64-byte NUL-padded slot** (`STRING_SHM_BYTES`, matching `KRON_SYS_TEXT_LEN`). The slot holds the **TEXT, not the pointer** — an address from the runtime's memory means nothing to KronServer — so `plc_shm_sync` copies the characters and **ignores the force flag**, while `plc_shm_pull` skips text entirely: writing into a `char *` the runtime does not own is refused in KronServer's `encodeValue`, in `writeHotSwapVariable` and in the UI. ⚠️ **Both copies of the sync/pull codegen now come from `shmSyncLine`/`shmPullLine`** (the per-task twins live in `generateMainLoop`, which only receives `shmEntries`, hence `isString` on the entry). ⚠️ **The legacy `/proc/mem` sim still SKIPS text** (`typeSize` returns 0): it reads `PlcState` directly, where the field is an 8-byte pointer — only the mirror pollers use `newMirrorPollSpec`, which takes the width from the variable table. STRING stays out of `IEC_TYPE_PREFIX` (unaddressable, so never in the capture ring) and out of `IEC_CAST_C` (a cast to text is meaningless). `VarString` is KronServer's only variable-width type, so its `Size` is validated by range, not against `typeSizeMap`.

⚠️ **The 18 STRING conversions are `static inline` in `kronconverter.h`, which is GITIGNORED and REGENERATED by Build Libraries** — mirror them into `KrontekLibraries` or the next run deletes them and a fresh clone has none. (`__kron_streq` deliberately stayed in the tracked `kronsystem.h`.) `STRING` is in `_CONV_TYPES`, so the pairs get the normal FB tables, but **both directions are emitted BY NAME, never through the generic C-cast branch** — `(int32_t)(char*)` reads the pointer and `(char*)(int32_t)` invents one. ⚠️ **`x_TO_STRING` needs somewhere to write, and every ownerless answer is wrong** (a static races between task threads, a thread-local pool goes stale, a compound literal dangles after the scan), so the transpiler gives **every call site its own `char[64]` PlcState field** (`strConvSymbol`): ladder sites are named after the block instance, ST sites are numbered against an UPPER BOUND from `countStringConvCalls` (comments and literals are stripped before the rewrite, so it consumes fewer — over-allocating wastes 64 bytes, under-allocating would name a field that does not exist, which is a compile error). One counter per POU, since an SCL program calls `transpileSTLogics` once per ST rung. ⚠️ **Programs only** — a FUNCTION/FB has no PlcState field, and both paths say so loudly.

⚠️ **`=`/`<>` on STRING are LOWERED to `__kron_streq`** in both ST and the ladder `EQ`/`NE` blocks — plain C `==` on two `char *` tests ADDRESSES, so equal text in different buffers compares false and nothing warns. `KRON_EQ`'s `_Generic` would pick the numeric overload, and `kroncompare.h` cannot hold the fix (Build Libraries regenerates it; only `kronsystem.h` is tracked), so the **transpiler** decides by operand type. Ordering (`<`, `>`) on text is a hard error, not a silent pointer compare.

⚠️ **Three constraints on the bodies** (the header's own comment block is the long form): `us_tick` is **invisible** inside a `kron*.h` — its extern is emitted *after* `customIncludes` — and its origin is platform-dependent, so blocks read `CLOCK_MONOTONIC` via `__kron_mono_us()`; an FB instance is a PlcState field, **zero-filled with no way to set a non-zero initial**, hence the `__primed` bool instead of a `-1` sentinel; a **Retain**ed instance carries a **stale monotonic timestamp** across a restart, so `Hour_Meter` drops gaps > 10 s rather than booking downtime as run time and every polled block discards a cache deadline further away than one poll period (`__kron_sys_cached`) — a future deadline from the previous boot would otherwise freeze it for that whole gap.
...
⚠️ **A non-Krontek dependency of a library repo goes in `externalLibDeps`** (`libraries_kron.go`): this builder compiles top-level `.c` directly and never runs the repos' CMake, so a `FetchContent` dep is simply absent — KronMotion's `TrajectoryGenerator.h/.c` is cloned there, staged as a header and built as its own `libTrajectoryGenerator.a` (everything in `lib/` is linked).
...
- ⚠️ **`runWithTimeout` APPENDS to `cmd.Env`, never resets it** — `runUpdateServer` sets `GOOS`/`GOARCH`/`GOARM`/`CGO_ENABLED=0` first; resetting silently ships host-arch dynamic binaries to ARM boards.

## 16. Lossless capture ring
`GET /variables` returns only the latest value and `/stream/buffered` server-samples at ~1–1.5 kHz max on an SBC (Go ticker), aliasing a 100 µs task. The ring delivers EVERY kept scan of the **addressed** variables by flipping to **producer-push**: the scan loop itself appends. ⚠️ **The byte layout is the contract in [`server/RING_FORMAT.md`](server/RING_FORMAT.md)** — the C codegen and `server/ipc_ring.go` must match exactly.
...
- ⚠️ **Size = `min(P × 10 s, RingRAMPercent × MemAvailable)` — the percentage is a CEILING, not the target** (`ringBytesFor`). `P` comes from the deployed variable table (`ringProducedBytesPerSec`, using **`RecordStride`** not `payload_len` — a record occupies a whole slot). Bigger is NOT better, for **latency, not memory**: tmpfs slots cost no RSS until filled, but the controller trips on `fill = backlog/nslots`, a *fraction* — at 50 % of 3.4 GiB (76 M slots) a consumer falling behind stays invisible for ~6 min while its data goes that stale. 10 s of production absorbs a TCP hiccup and still surfaces sustained overload in ~5 s. `/status` reports `ring_produced_bytes_per_sec` so the editor divides for the real stall tolerance (guessing 8 B/var ignored the record header → 3× optimistic).
- ⚠️ **The controller's decrease must be MULTIPLICATIVE** (`api_ring.go`, `n -= n/10`). Growth doubles, so an additive `n--` per 100 ms tick could not undo it: a burst to `N=2048` needed 2047 ticks (**3.4 minutes**) to walk back and any blip on the way re-doubled it. Observed on a 100 kHz counter: stride pinned near 1500, **0.08 % of scans delivered**, `seq_gaps=0`/`dropped=0` (nothing *lost* — production was thinned at the source). ⚠️ It is a **one-way trap**: at high stride there is nothing left to send, so `D` collapses and `floor = ceil(P/(α·D))` keeps it high — `D` measures *utilisation*, not *capacity*, and a small ring pushes you in. (Driving it off the backlog trend would remove the trap — not done.)
...
- ⚠️ **The header is RE-PARSED on epoch change** (`syncLocked`) — a Build & Send that changes the addressed set restarts the runtime with a different `record_stride`/`nslots`; without re-parsing the agent decodes new records with the old stride (only the first variable appears). The producer publishes `epoch` LAST, release store (ARM is weakly ordered).
- ⚠️ **`Drain` is byte-bounded per frame (4 MiB)** — else a client attaching to a live high-rate producer drains the whole ring in one allocation and **OOM-kills the agent** (4.3 GB RSS on a Jetson). On lap it counts `dropped`, resuming at the oldest surviving slot.
- ⚠️ **`stride_N = 0` means PAUSED — the producer stops writing entirely**, set at boot and when the last consumer leaves, so the runtime burns zero cycles. `SetStrideN` must NOT coerce `0→1` (codegen is `if (N == 0) return;`). Trade-off: no pre-connect history.
**Proven** on real hardware (Jetson Orin Nano, 10 µs task, 0 drops). ⚠️ **Deploying is TWO steps** — Build & Send ships only the runtime; the ring endpoints live in KronServer, so run `server/build.sh` + Deploy Server to Target (a stale agent 404s `/api/v1/ring/info`). **Gaps (v1):** scalars only; hot-swap doesn't feed the ring.
...
- ⚠️ **The file is SELF-DESCRIBING and matched BY NAME** (`"KRTN" u16 ver u16 count`, then `{u16 nameLen, name, u32 size, bytes}`). A raw `memcpy(&PlcState)` blob would be shorter and catastrophically wrong — one added variable shifts every offset. Unknown/mismatched records are skipped; a new retained variable keeps its initial.
...
- ⚠️ **`g_retain_mu` makes that safe:** the flusher runs OUTSIDE the scan barrier, so it may sit in the old module's `plc_retain_save` when thread 0 `dlclose`s it — `do_swap` holds the same mutex across dlclose + re-resolve.
...
**Deliberate v1 limits — say these out loud:** the snapshot is async, not scan-boundary, so a >word-sized value on a 32-bit board can be sampled mid-update; the power-cut loss window is up to 1 s (vs per-change writes → flash wear); **Build & Send does NOT reset retained values** and there is **no "clear" button** (force/PULSE-write, or delete `retain.dat`). ⚠️ **Not yet run on real hardware.**
