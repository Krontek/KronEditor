...
- ⚠️ **Boot persistence is BEST-EFFORT** — minimal images lack systemd *and* `crontab`, so `installBootHook` tries `@reboot` cron, then `/etc/rc.local`, then returns `none` with a warning: never fail a deploy over it, the agent runs regardless. SSE topic `server-deploy-progress`.
...
**Implementing one: `kronsystem.h` is the pattern** — other blocks live in a prebuilt `libkron*.a` and §1 forbids generating `.a`s, so this header holds `static inline` bodies compiled into `plc.c`/`logic.so`. ⚠️ It is the ONE file under `krontek-include/` besides `HAL/` that is **git-tracked**. It carries **25 blocks**: RTC/calendar, scheduling (`Time_Switch`, `Daily_Trigger`, `Astro_Clock`), diagnostics (`Read_Uptime`, `Cycle_Time_Monitor`, `Hour_Meter`, `Watchdog`), Linux-only host health (temp/load/disk, else `ERR_ID=1`), TIME arithmetic, `Blink`/`Debounce`/`Gen_Signal`. **Five** things must line up — the XML and a `libraryTree.js` entry (else it never reaches the Toolbox), plus:
...
- ⚠️ **Gated `#if defined(__linux__) && !defined(PLC_HOTSWAP)`** (KronServer is the only consumer; a hot-swap `.so` doesn't own this memory) — Windows/macOS sim gets no-op stubs, and a "Go Live" project does NOT feed the ring.
- ⚠️ **Segment sizing is KronServer's, not the runtime's, and MUST hang off `pm.preStartHook`** (`main.go` → `Server.SizeRingSegment`, `ringsize.go`): it `ftruncate`s `<shm>_ring` before the spawn so the runtime *adopts* the size (it sizes the segment itself only when smaller than a record), needing no env across `sudo`; `epoch` bumps at init so a re-attached consumer resets its cursor. ⚠️ **`StartRuntime` is only ONE of four spawn paths** — the ConnectRPC `Start` (**the editor's Run button**), AutoRun (`main.go`) and the crash watchdog (`process.go`) call `pm.Start()` directly, so sizing there alone left Run on the runtime's **1 MiB** fallback (0.43 s at 100 kHz) while `/status` reported the configured size. `preStartHook` is the one choke point all four share (it already carries `WriteInitialValues`).
- ⚠️ **Size = `min(P × 10 s, RingRAMPercent × MemAvailable)` — the percentage is a CEILING, not the target** (`ringBytesFor`). `P` comes from the deployed variable table (`ringProducedBytesPerSec`, using **`RecordStride`** not `payload_len` — a record occupies a whole slot). Bigger is NOT better, for **latency, not memory**: `/dev/shm` is tmpfs and the producer never initialises slots, so an oversized segment costs no RSS until it fills, but the controller trips on `fill = backlog/nslots`, a *fraction* — at 50 % of 3.4 GiB (76 M slots) a lagging consumer stays invisible for ~6 min while its data goes that stale. 10 s of production absorbs a TCP hiccup and still surfaces sustained overload in ~5 s. `/status` reports `ring_produced_bytes_per_sec` so the editor divides for the real stall tolerance instead of guessing from the variable list (the old guess assumed 8 B/var, ignoring the record header → 3× optimistic).
- ⚠️ **The controller's decrease must be MULTIPLICATIVE** (`api_ring.go`, `n -= n/10`) — growth doubles, so an additive `n--` per 100 ms tick never undid it: a burst to `N=2048` took 2047 ticks (**3.4 min**) to walk back, re-doubling on any blip. Seen in the field on a 100 kHz counter: stride pinned near 1500, **0.08 % of scans delivered**, `seq_gaps=0`/`dropped=0` (nothing *lost*; production thinned at the source). ⚠️ A **one-way trap**: at low stride `D` measures the link, but at high stride there is nothing left to send, `D` collapses, and `floor = ceil(P/(α·D))` keeps it high — `D` measures *utilisation*, not *capacity*. A small ring pushes you in.
- ⚠️ **The controller's `P` must be `WireBytesPerSec`, not `ProducedBytesPerSec`** — `D` is measured over whole frames, so `P` includes the 12-byte per-record *wire* header (`u64 seq + u16 task_id + u16 payload_len`; distinct from the 16-byte *slot* header). Payload-only understated production 2.5× for one 8-byte variable, so `floor` came out that much too low; `ProducedBytesPerSec` stays payload-only for `/ring/info`.
