...
- ⚠️ **Anthropic prompt caching:** `buildSystemPrompt` must stay **stable**; volatile project state rides the separate `context` field, appended AFTER the `cache_control` breakpoints — caching is a byte-exact prefix match.
...
- **Gap:** EtherCAT/motion are NOT hot-swap-trampolined (§6) — such a project needs a full redeploy + restart.
...
⚠️ **An XML block with no C implementation is a PHANTOM — draggable, then unbuildable.** Nothing checks that `<Block type="X">` has a C struct + `X_Call`, so it transpiles happily and dies at **compile** time with `unknown type name 'X'`. **Still phantom:** `Log_Data`, `PID_Compact`, `Filter_LowPass`, `Modbus_Master`, `MQTT_Client`, `TSEND`, `TRCV`, `Get_Alarm_State`, `Read_Hardware_ID`. Comparison/math/bitwise/trig/`X_TO_Y` legitimately have no `_Call` (emitted inline).

**Implementing one: `kronsystem.h` is the pattern** — other blocks live in a prebuilt `libkron*.a` and §1 forbids generating `.a`s, so this header holds `static inline` bodies compiled into `plc.c`/`logic.so`. ⚠️ It is the ONE file under `krontek-include/` besides `HAL/` that is **git-tracked**. Four things must line up:
1. **The header** — `typedef struct {…} X;` + `static inline void X_Call(X *inst)`; `transpileToC` scans every served `kron*.h` for that shape and auto-registers it as an FB. ⚠️ The parameter list must not contain the literal text `TIME` or the block is taken for a timer and called as `_Call(&inst, us_tick)`.
2. **The four transpiler tables** (`FB_TRIGGER_PIN`, `FB_Q_OUTPUT`, `FB_OUTPUTS`, `FB_INPUTS`) with pin names exactly as the XML declares.
3. **`SYSTEM_FB_TYPES`** if the trigger pin is `EN` — else `isInlineMathType` calls it stateless inline math and it gets no `PlcState` field, no instance and no codegen branch to land in.
4. **`getOutputPinType`** for any pin the generic rules mistype (`Read_System_Time.TIME` is a **DINT**, not an IEC duration — without it, silently BOOL). That block reports **ms since LOCAL midnight** (epoch ms wouldn't fit a DINT) via `localtime_r`/`localtime_s`, since several task threads can call it in one scan.
...
- ⚠️ **Never call `navigator.clipboard.readText()` while OPENING a menu** — Firefox/Safari answer with a native "Paste" chip and the app menu appears only once it is dismissed. Menu builders use `peekClipboard()`/`refreshClipboard()`; **cross-window sync rides a localStorage mirror**, so same-origin windows see each other permission-free.
...
- ⚠️ **Segment sizing is KronServer's, not the runtime's** — `StartRuntime` `ftruncate`s to `RingRAMPercent` × `MemAvailable` (default **50 %**) BEFORE spawning, so the runtime adopts it and no env must survive a `sudo` reset; it bumps `epoch` at init so a re-attached consumer resets its cursor.
