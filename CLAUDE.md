...
- ⚠️ **Segment sizing is KronServer's, not the runtime's.** `StartRuntime` (`server.go`) `ftruncate`s `<shm>_ring` to the byte size from `RingRAMPercent` × `MemAvailable` (`ringsize.go`, clamped `[64 KiB, min(512 MiB, 25% avail)]`) BEFORE spawning the runtime, so the runtime *adopts* that size (fstat) — no env has to survive a `sudo` reset. The runtime only `ftruncate`s (to `KRON_RING_BYTES` or 1 MiB) when it finds the segment smaller than one record (standalone/local run). It bumps `epoch` at init so a re-attached consumer resets its read cursor.

**Consumer (`server/ipc_ring.go` + `api_ring.go`).** ONE shared `RingConsumer` mmap serves all stream connections (each keeps its own local `readSeq`; `Drain` is read-only on the map — only the controller writes `stride_N`).
...
**RAM-% UI** (`SettingsPage.jsx`, Connection tab → "Capture Buffer"): reads `/status`'s `mem_total_bytes`/`mem_available_bytes`/`ring_ram_percent`/`ring_bytes`, lets the user set 0–25 %, POSTs `{ring_ram_percent}` to `/deploy/config`. ⚠️ **The primary unit the user reasons in is % of RAM but the data rate is tiny** (~80 KB/s per LINT @100 µs) — the panel also shows resolved bytes and ≈ seconds-of-history so a naive % doesn't silently reserve minutes of buffer.

**Verification.** `server/ipc_ring_test.go` + `api_ring_test.go` (synthetic C-format producer → Go drain: in-order/no-loss, lap-loss counting, stride round-trip, HTTP e2e). Proven end-to-end on Linux: real transpiler → bundled clang → running runtime → real KronServer HTTP → `tools/ring_stream_client.py` reassembled **39 663 records over 3 s, seq gaps 0, dropped 0, stride_N 1** (100 µs task, LINT @%ML0). ⚠️ **Not yet run on the physical device** — deploying a new transpiler runtime + rebuilt KronServer is the user's Build & Send step; the on-device acceptance test remains.
