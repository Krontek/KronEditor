...
**Local sim (host-agent)** — `host-agent/hotswap.go`: `POST /api/host/hotswap/{build,run,swap,stop}` compiles host (once) + `logic_<n>.so` (`-shared -fPIC -DPLC_HOTSWAP` + libs), runs the host, pushes swaps (write `swap_request` + SIGUSR1), and a `/dev/shm` poller streams live vars on the existing `simulation-output` SSE. **Each swap compiles a fresh `logic_<ver>.so` (ver++), so to stop the build dir filling up `removeStaleLogicSO(dir, keepVer)` runs after every compile (`compileLogic`/`compileLogicForTarget`) and deletes all `logic_*.so` EXCEPT the just-built `keepVer` and the base `logic_0.so` (kept because a fresh run/restart reloads it). Deleting a still-`dlopen`'d .so is safe on Linux. The on-target KronServer (`server/hotswap.go` `handleDeployLogic`) does the SAME cleanup in its deploy dir.** HostClient: `hotswapBuild/Run/Swap/Stop`. App.jsx: `startHotSwapSession`/`stopHotSwapSession` + `handleAgentHotSwap` (re-transpile → `hotswapSwap`). **HOT-SWAP IS NOW THE DEFAULT RUNTIME — there is no separate "Go live" session/button.** The old "Go live" button was removed. **Hot reload is now a SIMULATION-ONLY default; remote/field hot reload was tried and REVERTED.** (1) **Simulation ON** (`handleToggleSimulation`) transpiles with `simMode=true` and calls `host.hotswapBuild` (NOT `compileSimulation`/`writePlcFiles`) then `runSimulationNow` → `host.hotswapRun`; it sets `hotSwapActiveRef.current=true`, `setIsHotSwap(true)`, `taskSigRef`. Stop (`handleStopExecution` sim branch / toggle OFF) → `host.hotswapStop` + clears those flags. (2) **Build & Send** (`handleBuildAndSend`) deploys a SELF-CONTAINED runtime via `host.compileForTarget` + `deployToServer` — a full deploy + RESTART (NOT hot-swap), and it does NOT set `isHotSwap`. (3) **On an agent-approved change while a SIM is running, `handleAgentHotSwap` ASKS first** (`window.confirm` "Hot reload now?") via the sim branch (`hotSwapActiveRef`) → `hotswapSwap`. The field branch (`isHotSwap && isPlcConnected`) exists but is now UNREACHABLE (nothing sets `isHotSwap` for a remote target), so a connected-PLC agent change just commits to the project (deploy via Build & Send). Decline keeps the edit. The task-signature guard still refuses a sim swap when task timing changed. **Live MONITORING (`read_live_variables`/`watch_live_variables`) is independent of all this — it reads the `liveVariables` SSE stream that flows for any run (sim or connected).** Caveat: in hot-swap sim, FB instance outputs stream as FLAT scalar keys (`prog_X_blink.Q`, read from `/dev/shm` by offset) rather than DWARF-decoded objects — same representation as the connected-PLC case; the ST overlay/hover handle flat keys, but the watch table shows them as individual pin rows, not a combined `{Q,ET}` object.

**Why remote hot reload was reverted (the loader-host arg crash):** making Build & Send deploy the loader-host (`hotswapTargetBuild`) broke the remote — `deployToServer` pushed `logic_0.so` via `POST /deploy/logic`, but KronServer's `handleDeployLogic` does `ver++` and saved it as **`logic_1.so`**, while `process.go` checks for `logic_0.so` specifically → not found → `hotSwap=false` → started the loader-host `runtime.bin` with NO logic argument → `host.c main` `if (argc<2) return 1` → **immediate exit** ("runs but stops instantly"). Resolution (deliberate user choice — the field path was never HW-verified anyway): Build & Send reverted to `compileForTarget` (self-contained runtime), and **`deployToServer` no longer uploads `logic_0.so` at all** (the sim hot-swap leaves an x86 `logic_0.so` in the SAME build dir; shipping it to an ARM target would wrongly flip host mode). A single re-Build & Send overwrites the bad loader-host on the target with the plain runtime (the stale `logic_1.so` left on the device is harmless — `process.go` only looks for `logic_0.so`). To re-enable remote hot reload later, fix the base-logic naming (e.g. `/deploy/logic?base=1` → save as `logic_0.so` + reset the counter) AND verify the loader-host on real hardware.
...
**Edge cases requiring a cold restart, NOT a hot-swap** (editor must detect via a layout/ABI signature): task-config change (count/interval/priority), any variable-table change (add/remove/retype/re-address a var, esp. globals), FB instance add/remove/retype, board/IO/HAL config, EtherCAT/PDO, UDT change, SHM size. Rule: hot-swap only when the `PlcState` layout + variable table + task/IO/EC config are byte-identical to the running one; only POU logic bodies differ. **Task-scheduling part of this is now ENFORCED** (`App.jsx`): a module-scope `taskSignature(struct)` (JSON of each task's `name`+`interval`+`programs[{program,priority}]`) is snapshotted into `taskSigRef` when a hot-swap session STARTS; `handleAgentHotSwap` refuses to push the online change (sim OR field) when the current signature differs — it `addLog`s a warning and returns WITHOUT swapping, so the edit stays accepted in the project but the user must Build & Send (cold restart) to apply task-timing changes. Task durations deliberately do NOT hot-reload. (The other layout signatures — variable table / FB / IO / EC — are not yet signature-gated this way; they still rely on the runtime rolling back a bad swap.)
