...
16. [Lossless capture ring](#16-lossless-capture-ring-every-scan-value-over-the-rest-api)
17. [Retentive (RETAIN) variables](#17-retentive-retain-variables)
...
- **`serverBinaryForBoard` (deploy_ssh.go) maps board-id prefix → server binary arch.** aarch64 board prefixes → `plc-agent_linux_arm64`; `bb_` (armv7) → `plc-agent_linux_armv7`; default → amd64. ⚠️ When adding a new aarch64 board family, its prefix MUST be added here or the deploy ships the wrong-arch binary. The binary source is `resources/<triple>/server/plc-agent_linux_{arm64|armv7|amd64}` — `server/build.sh` copies its `dist/` output there, **so after editing `server/` you MUST re-run `server/build.sh` or the deploy ships a stale binary.**
  ⚠️ **`server/build.sh`'s copy-into-`resources/` step used to silently no-op when the destination didn't exist yet** (`if [ -d "$dst_dir" ]` with no `mkdir -p`) — so on a machine that never clicked "Build Server" from Settings → Libraries even once (the Go path, `libraries_server.go`, DOES `os.MkdirAll` first and was never affected), `resources/<triple>/server/` doesn't exist, the shell script's Go cross-compile succeeds and `dist/` gets fresh binaries, but the sync into `resources/` — the ONLY place `deploy_server_to_target` reads from — does nothing, with **zero error or warning printed**. The only visible symptom is "Deploy Server to Target" refusing with *"Server binary not found ... Build the server first (server/build.sh)"* immediately after a successful, freshly-run `server/build.sh` — which reads as the tool being wrong, not as a missing directory. Fixed: the function now `mkdir -p`s the destination unconditionally, matching the Go path's behavior.
...
resources/krontek-include/    SINGLE shared header tree (HAL/*.h + kron*.h + soem/) — arch-independent, served to EVERY target via one -I (host-agent paths.go ResourceTargetIncludeDir). Replaces the old per-triple include/ copies.
  kronsystem.h                git-TRACKED (like HAL/): header-only `static inline` blocks that need no libkron*.a — see §14
...
- **Continuation merge:** lines ending in an operator / `,` / `(` merge with the next. `:=` is excluded. **A line that STARTS with a binary operator (`+ - * / < > <= >= <> =`, `AND OR XOR NOT MOD BAND BOR BXOR`) also merges into the previous line** — no valid ST statement begins with a binary operator, so once a bare CASE label (`n:` / `name:`, detected by a top-level `:` not followed by `=`) and a line ending mid-block (`THEN`/`DO`/`OF`/`ELSE`) are excluded from the check, this is unambiguous. This exists because the two directions are NOT symmetric in the code: the original merge only looked at whether the line so far *ends* in a continuation token; it never looked ahead at whether the *next* line *starts* with one. A statement like `angle_raw := a * 128\n  + b / 2;` (operator leading the continuation line — legal, common IEC style) was silently split into two C statements: the assignment lost its second term, and the orphaned `+ (...);` became a discarded-value expression (`-Wunused-value`) — a real logic bug, not just a compiler nag. Same failure mode with a leading `AND` chain (`ready := a\n  AND b;`), except the orphaned line began with `&&` in the emitted C, which clang rejects outright (`error: expected identifier`). Verify any future change here against both a leading-`+` and a leading-`AND`-chain fixture, plus a `-1:`-style negative CASE label to confirm labels still don't get swallowed.
...
### ⚠️ INVARIANT — IEC identifiers are CASE-INSENSITIVE; the editor must match that (`src/utils/iecNames.js`)
The transpiler resolves ST names through a lowercase map behind a `/gi` alternation regex (`varMapLower`, `resolveVarsInExpr`) and `stValidation` matches `allowedLower`, so `var0 := var0 + 1;` against a declared `Var0` **compiles, runs and counts up correctly**. Every editor-side comparison of a *typed* name against a *declared* one must follow the same rule, via the shared helpers — `liveGet` / `liveResolveKey` / `memberGet` / `liveEntriesWithPrefix` for live keys, `findVarByName` / `hasVarNamed` for the variable list. **Never `liveVariables[key]` or `.find(v => v.name === typed)` on a user-typed name.** Two distinct failures came from exact compares:
- **Silent live blackout.** A live key is built from the DECLARED spelling (`prog_<POU>_Var0`), so looking it up with the typed spelling found nothing: the ST/SCL badges, ladder value overlays and **live power flow** all rendered as if the variable did not exist, while the variable table (which keys off the declaration) showed a happily incrementing value. That reads as "live view is broken", not as "the casing differs" — and there is no error anywhere, because the program itself is correct.
- **Duplicate-variable corruption (worse).** LD inline auto-declare's exact existence check created a SECOND variable for a case-variant reference (`var0 : BOOL` beside `Var0 : LINT`). Both reach the transpiler, which emits two distinct `PlcState` fields but collapses the names in `varMapLower` (last wins) — so the ladder block and the ST code silently wired to **different storage**.
- ⚠️ A force-write addresses its slot **by key**, so those paths need `liveResolveKey` (the stored key), not `liveGet` (the value) — passing the typed spelling on writes to a slot that does not exist and silently does nothing.
- The lowercase index is cached per live-values object in a `WeakMap`; a fresh object arrives every SSE frame, so a per-lookup scan would be O(identifiers × variables) per frame.
...
- **`src/services/agentTools.js`** — the pure action surface. `TOOL_DEFS` + `applyToolCall(struct, name, args)` returns `{mutation, ok, summary, diff, next}` (never mutates in place). Tools: `get_project_overview`, `read_pou`, `list_blocks`, `create_pou`, `rename/delete_pou`, `set_st_code`, `add/update/remove_variable` (incl. `retain` — §17), `set_ladder`, `create_data_type`.
...
- ⚠️ **A DEAD turn — the model returns no text AND no tool calls — must never reach the transcript.** It used to fall through `if (calls.length === 0) { setRunning(false); return; }` as a "final answer", and since the `pushView` above it is skipped for empty text, **the agent simply went quiet mid-task with nothing on screen**; worse, the empty turn was appended to `convoRef` *before* that return, and `callAnthropic` serialized it as `{"type":"text","text":""}` → `400 "messages: text content blocks must be non-empty"`. Because the dead turn stays in history, that 400 then repeats for **every later message**: one empty model reply **bricks the whole conversation**, not just a turn (observed end-to-end: dead turn, then the user's "yaptin mi?" and everything after it 400'd). Fixed in three places, and the load-bearing one is the send-time repair because it also rescues an ALREADY-damaged history: `providerSafeMessages` **drops** dead assistant turns (nothing references them — a turn with no tool calls has no tool results) and `callAnthropic`/the user branch **skip** empty text blocks entirely rather than padding with `""`; `runTurn` retries once on the same history and then says so out loud. `host-agent/ai_anthropic_test.go` pins the serializer (it fails on the old code with the exact empty-block finding).
- ⚠️ **A truncated turn is the OTHER source of a dead turn, and `max_tokens` was 4096 — far too small.** `stop_reason` was never parsed, so a reply the provider itself flagged as incomplete was handed back as if it were the model's answer. Two shapes, both verified live against `claude-opus-5` on subscription OAuth:
  - **Reasoning tokens bill against `max_tokens` and come back as a `thinking` block.** A turn that spent the whole 4096 thinking returned `content:[{"type":"thinking"}]` — **no text, no tool_use** — i.e. a dead turn produced by a perfectly healthy history, which the previous bullet's defenses can only mop up, never prevent. Observed end-to-end: the analysis reply was cut mid-word (`…POU'n`), then "devam" returned an empty bubble twice (the loop's one retry) and the run died. ⚠️ Note thinking is emitted **without** the `thinking` request param — you cannot conclude "we never asked for it, so it isn't there"; read `usage.output_tokens_details.thinking_tokens`.
  - **A tool call cut mid-arguments is returned PARTIALLY PARSED**, with the unfinished keys simply absent — a `set_st_code` came back as `{"pou":"esc_control"}`, no `code`. It therefore reaches `applyToolCall` looking well-formed. Here it happened to hit a required-field check (`set_ladder` → "rungs must be a non-empty array", which reads as a model mistake, not a truncation); a half-generated `rungs` array would have **passed** that check and replaced the POU's whole ladder with the fragment.
  `defaultMaxTokens` is now **32000** (`ai.go`) — the budget must hold thinking *plus* a whole-POU rewrite; the real `esc_control` rewrite measured **5333 output tokens**, so 4096 could never have carried this app's central operation. `callAnthropic` (`stop_reason == "max_tokens"`) and `callOpenAI` (`finish_reason == "length"`) now **discard the turn and fail with an explained error** rather than returning partial content or an executable partial tool call. Since the model list is a LIVE catalogue, there is no per-model cap table: a provider that rejects the budget names its own limit (`max_tokens: 32000 > 8192, …`), and `parseMaxTokensLimit` clamps + retries **once**. Only `callAnthropic` ever sends `max_tokens`; the OpenAI/Ollama paths rely on the provider's default. `host-agent/ai_truncation_test.go` pins all of it.
- ⚠️ **`parseArgs` MUST return a copy.** It returned the stored object when the arg was already parsed, and the turn loop enriches args **in place** (`args.__library = libraryData`, `__live`, `__watch`, `__board`) — so the enrichment was written straight into the assistant message living in `convoRef`. Two `list_blocks` calls in one turn glued a ~47 KB block catalog into history **twice** (measured: history went 40 KB → 137 KB in ONE turn), which is then re-sent on every later request and destroys the Anthropic prompt cache. The empty response that bricked the conversation above arrived on the very next turn. `cleanToolArgs` strips `__`-prefixed keys in `providerSafeMessages` as well, so histories already poisoned by the old build shrink back down instead of needing a reset.
...
⚠️ **Device-node overrides (UART + I2C + USB) — how they reach the HAL.** The interface-config Device Path/Node field (stored per port in `content.deviceInterfaceConfig`, resource id `res_config`) is emitted by `buildRuntimePortHelpers` (CTranspilerService.js) as `#define KRON_UART<n> "<path>"` / `#define KRON_I2C<n> "<path>"` / `#define KRON_USB<n> "<path>"` *before* `kron_hal.h`, and the HALs resolve the open path through `_*_devnode(ch)` (`_i2c_devnode` in kronhal_jetson.h, `_rpi_i2c_devnode` in kronhal_rpi.h) or a macro-built table (`_usb_devs[]`, `_uart_devs[]`). SPI has no override (path derived from bus/cs).

⚠️ **The USB override is gated on the port being ENABLED — UART/I2C are not.** The Device Path input is only *rendered* for an enabled USB port (`UsbPortsVisual`, BoardConfigPage.jsx), so a disabled port's stored path is config the user can neither see nor edit; letting it steer codegen means a stale leftover silently redirects a channel. Real projects carry exactly that shape (a port tried, given a path, then switched off), which would have pointed USB0 at another port's dongle. Unlike UART there is **no `KRON_USB_PortEnabled` gate in the HAL** — `_usb_open` opens whatever channel a block names — so this emission guard is the only thing standing between an invisible setting and a silently wrong device.

⚠️ **USB channel → default path is NOT `USB<n>` → `/dev/ttyUSB<n>`.** Jetson/RPi map USB0/1 → `ttyUSB0`/`ttyUSB1`, **USB2/USB3 → `/dev/ttyACM0`/`/dev/ttyACM1`**, USB4 → `ttyUSB2` (BB: USB0 → `ttyUSB0`, USB1 → `ttyACM0`). So **no channel defaults to `/dev/ttyUSB3`**, and on a board with no ACM devices, picking USB3 opened a path that did not exist: `open()` failed → `ERR_ID=2` → `NewData` false forever, with a working `ibus_read`-style tool on the same port proving the wiring fine. Before the override existed, `devicePath` was parsed into `usbPorts[]` and then simply never emitted, so the field was decorative and this was unfixable from the UI. The `#ifndef KRON_USB<n>` guards were already in all three HALs — only the emitter was missing. Verified by preprocessing the real `kronhal_jetson.h` with the generated defines: `_usb_devs[3]` becomes the configured path. Prefer a stable `/dev/serial/by-id/…` or `by-path/…` name here — `ttyUSB<n>` numbering follows enumeration order and moves across reboots; `_usb_open` does not cache a failed open, so a udev symlink that appears late self-heals on a later scan. This matters on Jetson Orin, where header pins 3/5 are I2C8 (typically `/dev/i2c-7`), not the table's Nano-era `/dev/i2c-1` — set the Device Node field after verifying with `i2cdetect -l` on the target. The I2C **CLOCK HZ field is decorative** — Linux i2c-dev cannot change bus speed at runtime (device-tree fixed); only the SPI clock is real (`KRON_SPI_PortResolve`).
...
An IEC address in the VariableManager "Address" column (`%MW0`, `%MX0.1`, …) exposes a variable via REST. Size prefix per type (`IEC_TYPE_PREFIX`, `src/utils/iecAddress.js`): BOOL→`%MX{byte}.{bit}`, BYTE/SINT/USINT→`%MB`, INT/UINT/WORD→`%MW`, DINT/UDINT/DWORD/REAL/TIME→`%MD`, LINT/ULINT/LWORD/LREAL→`%ML`. A plain number auto-formats by type. The single API password (SettingsPage → Connection) is SHA-256+salt-embedded into `variable_table.json`; empty = API disabled.

### ⚠️ INVARIANT — the address prefix MUST agree with the type, and nothing downstream re-derives it
An address is a **width claim**, but it is only a *label* to the runtime: the transpiler copies the string into `variableTable` verbatim, and KronServer reaches the variable by `Offset`/`Size` (both from the TYPE) while treating a non-empty `address` as nothing more than "expose over REST/HMI" (`ipc.go`). So a prefix that disagrees with its type **never fails anywhere** — the runtime works perfectly while the REST feed, the HMI variable picker, the ST/watch UI and the addressed-variable CSV all advertise a width the runtime does not serve. It surfaces only in the field, when the SCADA/Modbus side reads a word where a bit was published. The classic way in was changing a variable's TYPE after assigning its address; the old `formatIECAddress` also accepted **any** `%`-prefixed string verbatim (`%MZ999` included), and the type prefix was consulted only when the user typed a bare number.

`src/utils/iecAddress.js` is the single rule (`parseIECAddress` / `validateIECAddress` / `retargetIECAddress` / `formatIECAddress`), applied by every writer:
- **VariableManager** — validates on address entry (syntax + prefix↔type), and on a **type change retargets** the address keeping the operator-facing number (`%MW10` + BOOL → `%MX10.0`), clearing it if the new type is unaddressable or the rewritten address collides. ⚠️ Type + address must land in **ONE** update — `EditorPane.handleUpdateVar` and `ResourceEditor.handleUpdateVar` both accept an object patch for exactly this; two sequential single-field calls would expose the new type beside the old address (and ResourceEditor's non-functional `onContentChange` would drop the first write outright).
- **`agentTools`** — `add_variable` validates a supplied address against the resolved type; `update_variable` resolves the address **after** the type (explicit address → validated against the resulting type; type changed with the address left alone → retargeted).
- **The transpiler gate** (`transpileToC`, before any codegen) — hard `throw` listing every bad address across globals + program locals, in the same spirit as the array-bounds/SHM-overflow checks. This is the one that covers hand-edited project XML and projects saved before the checks existed; the UI paths can be bypassed, this cannot.

⚠️ **Only elementary IEC types can be addressed.** Arrays/structures/enums/FB instances/STRING are rejected: the debugDefaults expansion hands the SAME address string to every element/member, so an addressed `ARRAY[0..9] OF INT` published ten distinct REST variables all claiming one location. Keep `IEC_TYPE_PREFIX` in sync with the transpiler's four IEC type tables (§5) — a new primitive that gets a SHM slot but no entry here becomes silently unaddressable. `%I`/`%Q` locations parse (both are legal IEC and may exist in old projects); only `%M` is ever generated.

**Known residual gaps** (deliberate, not oversights): addresses on FUNCTION-BLOCK/FUNCTION locals are neither gated nor emitted — they silently do nothing; and duplicate addresses are blocked at UI entry and flagged with `⚠` in the ResourceEditor address map, but are **not** a build error, so a project that already contains duplicates still compiles.
...
### ⚠️ An XML block with no C implementation is a PHANTOM — draggable, then unbuildable
The XML + `libraryTree.js` only make a block appear in the Toolbox. Nothing checks that a `<Block type="X">` has a matching C struct + `X_Call`, so dropping a phantom into a rung transpiles happily (the LD path falls back to `X_Call(&inst)` and declares an `X` instance) and then dies at **compile** time with `unknown type name 'X'` / `implicit declaration of X_Call` — long after the point where the user chose the block, with an error naming C symbols they never wrote. `Read_System_Time` sat in the Toolbox like this for a long time.
- **Still phantom** (in the Toolbox, will not build): `Log_Data`, `PID_Compact`, `Filter_LowPass`, `Modbus_Master`, `MQTT_Client`, `TSEND`, `TRCV`, plus the two inline `libraryTree` items `Get_Alarm_State` / `Read_Hardware_ID`. Implement or delist before promising them.
- Comparison/arithmetic/bitwise/trig/`X_TO_Y` blocks legitimately have no `_Call` — they are emitted inline (`isInlineMathType`), so absence from the headers is expected for those and only for those.

**Implementing one: `resources/krontek-include/kronsystem.h` is the pattern.** Every other standard block is *declared* in a `kron*.h` and *implemented* in a prebuilt `libkron*.a` — and §1 forbids generating `.a`s, so a new block cannot go that route inside this repo. `kronsystem.h` holds `static inline` bodies instead, which compile straight into `plc.c` / `logic.so`; ⚠️ it is the ONE file under `krontek-include/` besides `HAL/` that is **git-tracked** (`.gitignore` negation), because a fresh clone must have it or projects using its blocks fail to compile. Four things must line up:
1. **The header** — `typedef struct {…} X;` + `static inline void X_Call(X *inst)`. `transpileToC` scans every served `kron*.h` for `<Type>_Call(<Type> *…)` and auto-registers the type as an FB (`stdFunctions`), so no include or registration is needed by hand. ⚠️ The parameter list must not contain the literal text `TIME` or the block is taken for a timer and called as `_Call(&inst, us_tick)` (`Read_System_Time` is mixed case, so it is safe — `READ_SYSTEM_TIME` would not be).
2. **The four transpiler tables** (`FB_TRIGGER_PIN`, `FB_Q_OUTPUT`, `FB_OUTPUTS`, `FB_INPUTS`) — pin names exactly as the XML declares them.
3. **`SYSTEM_FB_TYPES`** if the trigger pin is `EN` — otherwise `isInlineMathType` classifies it as stateless inline math and it gets no `PlcState` field, no instance and no shadow vars, and the LD path drops into the inline branch that has no `KRON_FN`/`BITWISE_OP`/`MATH_FB` entry for it. It is exported so `agentTools.js` can keep it in `STANDARD_FB_TYPES` and out of the "that's an inline math op" rejections in `resolveVarType` / `set_ladder`.
4. **`getOutputPinType`** for any output pin the generic rules would mistype (`Read_System_Time.TIME` is a **DINT**, not the IEC `TIME` duration its name suggests — without the entry it silently became BOOL).

`Read_System_Time` reads `CLOCK_REALTIME` and reports **milliseconds since LOCAL midnight** (0…86 399 999, the IEC time-of-day value) — deliberately not ms-since-epoch, which is ~1.7e12 and does not fit the DINT the pin is declared as. It uses `localtime_r` (`localtime_s` on Windows) because several task threads can call it in one scan and plain `localtime` returns a shared static. Verified end-to-end on Linux (transpile → bundled clang → running sim → `/dev/shm` mirror matched `date` to the millisecond, EN=false held the value) and compile-verified for all four target triples.
...
- ⚠️ **"Build SOEM" DOES build a real macOS `libsoem.a`** (host-only, like every other macOS library target — §15 "Target matrix"). Verified against the real `OpenEtherCATsociety/SOEM` v2.0.0 tree: it ships `contrib/oshw/macosx` + `contrib/osal/macosx`, a libpcap-based driver structurally identical to the win32/wpcap one `runBuildSoem` already built — it is simply not one of SOEM's OWN supported cmake targets (`cmake/Linux.cmake` and `cmake/Windows.cmake` exist, there is no `cmake/macosx.cmake`; the macOS port sits in `contrib/` alongside other unofficial ports for RTEMS/VxWorks/INtime/Erika). `runBuildSoem`'s `"macos"` case builds it: no extra `-I`/`-L` needed for pcap (headers/lib come from the Xcode SDK `bundledHostClangArgs` already points `-isysroot` at).
  - ⚠️ **The vendored `contrib/osal/macosx/osal.c` does NOT compile against SOEM v2.0.0 as cloned — this was discovered by actually attempting the build, not by reading the source.** It predates three real API changes: (1) `ec_timet` used to be a bespoke `{sec,usec}` struct, it is now `#define ec_timet struct timespec` (`.tv_sec`/`.tv_nsec`) everywhere including shared `src/ec_dc.c`; (2) `osal_get_monotonic_time`/`osal_monotonic_sleep` are declared in the shared `osal.h` and called from shared code, but were never implemented by the old file (a **link**-time failure a compile-only smoke test would miss); (3) `osal_mutex_create/_destroy/_lock/_unlock` likewise declared but unimplemented. `writeSoemMacosxOsal` (libraries_deps.go) replaces the file outright — adapted from `osal/linux/osal.c`'s POSIX primitives — rather than patching it; `patchSoemMacosxOsalDefs` separately completes `osal_defs.h`'s two missing macros the same way. **The one genuine platform difference:** `clock_nanosleep(CLOCK_MONOTONIC, TIMER_ABSTIME, …)` does not exist on Darwin at all (same gap the loader-host's `hotswap_host.c` hit for its high-resolution sleep, §6 "macOS simulation") — SOEM's own sleep isn't hard-real-time-critical enough to need `mach_wait_until`, so the portable fallback is used: convert the absolute monotonic deadline to a relative duration against "now" and `nanosleep()` that. `PTHREAD_PRIO_INHERIT` (used by `osal_mutex_create`) was verified to actually work on Xcode Command Line Tools clang rather than assumed.
  - `soemBuildInputs` (libraries_kron.go, used by "Build Libraries" when compiling `KrontekLibraries`' `kronethercatmaster.c` wrapper) has a matching `"macos"` case pointing at the now-real `contrib/osal|oshw/macosx` include dirs once headers are staged.
  - ⚠️ **This does NOT make local Simulation use real EtherCAT on macOS (or anywhere else).** `compileLogic`/`compileHost` (hotswap.go) pass `-DKRON_EC_SIM` **unconditionally, on every host platform** — local hot-swap simulation never talks to real fieldbus hardware, by design, regardless of whether a real `libsoem.a` exists for the host. Building the macOS archive only means: (a) "Build Libraries" can now compile the real (non-stub) `kronethercatmaster.c` wrapper for a macOS host target too, and (b) a future feature that deliberately wants local real-hardware EtherCAT testing from a Mac has the archive it would need — it is not blocked on SOEM upstream. Whoever links a real macOS EtherCAT master must add `-lpcap` at that final link step (the `.a` only references `pcap_*` as undefined symbols) and be aware opening a live capture on macOS typically needs elevated privileges (BPF device access).
  - `TestRunBuildSoemEndToEnd` asserts the darwin archive exists **only when the test itself runs on a Mac** (`runtime.GOOS == "darwin"`), same host-only rule as every other library target.
  - **⚠️ A `/deploy/project-file`-unrelated but adjacent gotcha this surfaced:** `kronethercatmaster.h` is unconditionally `#include`-d into every generated `plc.c` (§14 — every served `kron*.h` is, to auto-register its `_Call` FBs), and it always does `#include "soem/soem.h"` — so **every build, EtherCAT or not, needs `resources/krontek-include/soem/soem.h` to exist**, or clang fails with "soem.h not found" before it ever gets near actual project logic. A fresh clone / fresh machine that hasn't run "Build SOEM" yet hits this on the very first Build & Send or Simulation compile; running it once (headers are one shared arch-independent tree, §3) fixes every target at once.
...
## 16. Lossless capture ring (every scan value over the REST API)

**Problem it solves.** The plain REST poll (`GET /variables`) returns only the *latest* value, and `/api/v1/stream/buffered` *server-samples* at `interval_us` — but the Go ticker tops out ~1–1.5 kHz on an SBC, so a 100 µs (10 kHz) task's values get **aliased** (~1 in 7 captured). The **capture ring** delivers EVERY kept scan of the **addressed** variables with explicit loss accounting, by flipping the model from *server-pull-sampling* to *producer-push*: the PLC scan loop itself (already exactly on period, in C, SCHED_FIFO) appends each value.

⚠️ **The byte layout is the contract in [`server/RING_FORMAT.md`](server/RING_FORMAT.md)** — the C codegen (`CTranspilerService.js`) and the Go reader (`server/ipc_ring.go`) must match it exactly (same rule as the `/dev/shm` mirror paths). A second `/dev/shm/<shm>_ring` segment holds fixed-stride records; header at offset 0, records at 256.

**Producer (transpiler, `generateMainLoop`).** For each addressed SCALAR var (address set, real shm slot, plain c_symbol — no `[`/`.`), grouped by owning task (program-local → its program's task; **global → the FASTEST task**, so never undersampled; program unassigned to any task → skipped). Per task, `plc_task_body` calls `plc_ring_append_TaskN()` AFTER `plc_shm_sync` (values final): reads `stride_N` atomically, `if (scan_g++ % N) return;` else `__atomic_fetch_add(write_seq)`, memcpy its vars into slot `s % nslots`, publish `slot.seq` with a RELEASE store LAST. Wait-free (one relaxed add + one release store, never blocks the consumer). Multiple task threads are independent MPSC producers — `fetch_add` hands each a unique slot.
- ⚠️ **Gated `#if defined(__linux__) && !defined(PLC_HOTSWAP)`.** KronServer (the only consumer) is Linux-only; a hot-swap `.so` doesn't own this memory; the Windows/macOS SIM keeps compiling via **no-op stub** `plc_ring_*` functions in the `#else`. So a project using "Go Live" hot-swap does NOT feed the ring (documented limitation, like EtherCAT isn't trampolined) — a plain Build & Send device runtime does.
- `record_stride = align8(16 + max task payload)`; `variableTable.ring = {record_stride, tasks:[{task_id, period_us, vars:[{name,type,size}]}]}` is emitted so the CLIENT decodes payloads (KronServer forwards raw bytes, layout-agnostic).
- ⚠️ **Segment sizing is KronServer's, not the runtime's.** `StartRuntime` (`server.go`) `ftruncate`s `<shm>_ring` to the byte size from `RingRAMPercent` × `MemAvailable` (`ringsize.go`, clamped `[64 KiB, min(2 GiB, 50% avail)]`) BEFORE spawning the runtime, so the runtime *adopts* that size (fstat) — no env has to survive a `sudo` reset. **Default is 50% of available RAM** (`ringDefaultPercent`, applied when the config is unset) — cheap because `/dev/shm` is tmpfs/demand-paged so RSS only grows as the ring actually fills. The runtime only `ftruncate`s (to `KRON_RING_BYTES` or 1 MiB) when it finds the segment smaller than one record (standalone/local run). It bumps `epoch` at init so a re-attached consumer resets its read cursor.

**Consumer (`server/ipc_ring.go` + `api_ring.go`).** ONE shared `RingConsumer` mmap serves all stream connections (each keeps its own local `readSeq`). All `rc.mem` access is under `rc.mu` because the header may be re-mmap'd on a resize (below); ≤5 clients at 5 ms, so the contention is nil.
- ⚠️ **Header is RE-PARSED on epoch change** (`syncLocked`, called from `Drain` + the accessors). A Build & Send that changes the addressed-variable set restarts the runtime, which rewrites the header with a **different `record_stride`/`nslots`/task-table** and bumps `epoch`. Without re-parsing, the agent kept decoding the new 96-byte records with the old 24-byte stride → **only the first variable showed up, the rest were garbage** (the exact "I added 10 vars, 9 don't show" bug). If the segment also *resized* (new pre-size), `syncLocked` re-`fstat`s + re-`mmap`s. The producer publishes `epoch` LAST with a release store so a consumer that sees the new epoch also sees the fresh header (ARM is weakly ordered — load-bearing).
- ⚠️ **Resident memory tracks the live backlog, not the segment size** (`Reclaim`/`ReclaimAll` in `ipc_ring.go`). The RAM-% size is a **cap**, not a working size. Two pieces make RSS track the backlog: (1) the producer does **NO slot init** and `write_seq` **starts at 1, monotonic** (a hole reads 0 < every valid seq, so no `0xFFFF` init is needed — that init used to touch every page → whole segment resident at boot); (2) the consumer `fallocate(PUNCH_HOLE)`s the pages of records every client has drained (`minReadSeq`), each ~100 ms tick, and punches the whole area on PAUSE. Only slots both already-read AND not-yet-overwritten (`seq > write_seq - nslots`) are punched, page-aligned inward, so the wait-free producer is never raced. ⚠️ `madvise(DONTNEED)` does NOT free a shared tmpfs page — `fallocate` punch-hole is the tool. Consumer keeps up → RSS ≈ 0; falls behind → grows to the cap; idle → 0. The consumer starts `readSeq = write_seq` at connect (no pre-connect history; also never reads seq 0 from an empty slot).
- ⚠️ **`Drain` is byte-bounded per frame (`ringMaxFrameBytes` = 4 MiB).** A client attaching to an already-running high-rate producer would otherwise drain the WHOLE ring in the first tick — on a 2 GiB / 2.6 M-slot ring that is **gigabytes of allocation in one shot → the agent is OOM-killed** (observed on the Jetson: 4.3 GB anon RSS). The handler passes `maxRecords = 4 MiB / record_stride`; excess backlog waits for the next 5 ms tick (or laps → counted `dropped`, which is when the controller raises the stride).
- `Drain(readSeq)`: if `write_seq - readSeq > nslots` → lapped, count `dropped` and catch up to the oldest surviving slot; else read `[readSeq, write_seq)`, matching each `slot.seq == s` (ACQUIRE); a mismatch (`ss < s`, producer mid-write) stops the drain (retry next tick), never blocks.
- **`GET /api/v1/stream/ring`** pushes a binary frame every 5 ms: `u32 frame_len · u32 record_count · u32 stride_N · u64 dropped_total · records[{u64 seq,u16 task_id,u16 payload_len,payload}]`. **`GET /api/v1/ring/info`** returns the header + `variableTable.ring` layout for decoding.
- ⚠️ **Adaptive decimation controller** (server-owned, ~100 ms tick, off the real-time path — `ringController`): single global `stride_N` = **proportional "same ratio"** (every task keeps `1/N`). `N = ceil(P / (α·D))`, `α = 0.9` (stay strictly under the link so the ring never swells — `ceil` = round samples DOWN), `P` from the header task table, `D` = EWMA of bytes actually written to the client socket (TCP backpressure surfaces the link rate). **Fill-watermark emergency**: backlog fraction > 0.5 → `N *= 2` immediately (grow-fast, guarantees no unbounded swell even if `D` is mis-estimated); < 0.1 sustained and above the analytic floor → `N--` (shrink-slow). Loss (from lapping) is reported per frame, never silent.
- ⚠️ **`stride_N = 0` means PAUSED — the producer stops writing to the ring entirely.** When NO consumer is connected the controller sets `stride_N = 0` (last `unregister`), so the runtime burns zero scan cycles on `ring_append` and the ring stops filling RAM; `register` resumes to the last-good stride (≥1) immediately on connect. The runtime **boots paused** (`plc_ring_init` writes `stride_N = 0`). ⚠️ `SetStrideN` must NOT coerce `0→1` (it once did, for an "invalid" guard) — `0` is now a valid, meaningful value. The producer codegen is `if (N == 0) return;` (not `N=1`). Trade-off: no pre-connect history — a client sees data building from connect (a brief warm-up), which is the right default for live monitoring; the old "buffer always" behavior pinned the full ring RSS within minutes for data nobody read.

**RAM-% UI** (`SettingsPage.jsx`, Connection tab → "Capture Buffer"): reads `/status`'s `mem_total_bytes`/`mem_available_bytes`/`ring_ram_percent`/`ring_bytes`, lets the user set 0–50 % (`ringMaxRAMPercent`; the unset default is also 50), POSTs `{ring_ram_percent}` to `/deploy/config`. ⚠️ **The primary unit the user reasons in is % of RAM but the data rate is tiny** (~80 KB/s per LINT @100 µs) — the panel also shows resolved bytes and ≈ seconds-of-history so a naive % doesn't silently reserve minutes of buffer.

**Verification.** `server/ipc_ring_test.go` + `api_ring_test.go` (synthetic C-format producer → Go drain: in-order/no-loss, lap-loss counting, stride round-trip, HTTP e2e). Proven end-to-end on Linux: real transpiler → bundled clang → running runtime → real KronServer HTTP → `tools/ring_stream_client.py` reassembled **39 663 records over 3 s, seq gaps 0, dropped 0, stride_N 1** (100 µs task, LINT @%ML0). ✅ **Also PASSED on real hardware** (Jetson Orin Nano, aarch64, a **10 µs / 100 kHz** task): after attach, **0 streaming drops, 0 seq gaps, stride_N 1** at ~108 k records/s — the 800 KB/s link kept up so no decimation was needed. The one-time "attach backlog" (ring history lapped before the client connected) is reported separately from streaming loss by `ring_stream_client.py` and `ApiSamples/ml0_ring_viewer.py`. ⚠️ **Deploying to the device is TWO steps, not one:** Build & Send updates only the RUNTIME (producer). The ring ENDPOINTS live in KronServer, so the agent must ALSO be redeployed — run `server/build.sh` (else `Deploy Server to Target` ships a stale binary, §2) then Settings → Deploy Server to Target. Symptom of a stale agent: `/api/v1/ring/info` returns **404** (route missing) while `/dev/shm/<shm>_ring` is being written; a NEW agent returns 401 without a token.

**Known gaps (v1):** scalars only (array/struct members excluded); hot-swap "Go Live" doesn't feed the ring; multi-client decimates to the slowest link; globals sampled at the fastest task's rate.

---

## 17. Retentive (RETAIN) variables

A variable whose **class is `Retain`** keeps its LAST value across a runtime restart (IEC 61131-3 warm restart) instead of returning to its initial value. Everything retainable already lives in exactly one place — `PlcState` (§6) — so persistence is a name-keyed snapshot of a subset of its fields to **`retain.dat`** in the runtime's cwd: the deploy dir on a target (`process.go` `cmd.Dir`), the build dir for the local sim (`hotswap.go`).

### Where it is declared
| Scope | Offered? | Why |
|---|---|---|
| Global variables | ✅ `['Var','Constant','Retain']` (`ResourceEditor.jsx`) | field of `PlcState` |
| Program locals | ✅ `['Local','Temp','Retain']` (`App.jsx`) | field of `PlcState` |
| **FB instance** declared in a program | ✅ (it is just a program local) | the WHOLE struct is one blob → a `CTU` keeps its `CV`, a `TONR` its accumulated `ET` |
| Variables inside a function block / function | ❌ deliberately absent | they are FB struct members / C locals, **not** `PlcState` fields — the transpiler could not persist them, so offering the class there would be a silent no-op. Retain the INSTANCE in the declaring program instead. |

⚠️ **The Class column was hidden for exactly the two scopes that now need it.** `VariableManager`'s `showClass` only fired for POUs with pin classes (`Input`/`Output`/`InOut`), so the `Retain` option globals had been offered since long before this feature could never be selected — the whole thing was unreachable config. `showClass` must keep testing for `'Retain'`. Its sibling trap: the `<select>` falls back to `allowedClasses[0]` whenever the STORED class isn't offered in that scope (not merely when it's missing) — an agent-created global carried class `'Global'`, and a `<select>` whose value matches no option renders **blank**. (`agentTools.makeVar` now defaults globals to `'Var'`.)

### How it works (`generateRetainSupport`, CTranspilerService.js)
- `retainEntries` is collected next to `stateFields`; the emitted `__retain_tab[]` holds `{name, offsetof(PlcState,f), sizeof(f)}`, so **no size or type logic is duplicated** — an array, struct or FB instance persists by the same three lines as a BOOL.
- ⚠️ **The file is SELF-DESCRIBING and matched BY NAME** (`"KRTN" u16 ver u16 count`, then `{u16 nameLen, name, u32 size, bytes}`; scalars explicit little-endian, payload native — ⚠️ the VALUE length is 32-bit because a retained `ARRAY[0..20000] OF DINT` is 80 KB and a u16 would wrap silently). A raw `memcpy(&PlcState)` blob would be one line shorter and catastrophically wrong: adding one variable shifts every offset, so a rebuild would restore garbage into the wrong fields. Unknown record → skipped; size mismatch (retyped) → skipped; retained variable with no record (new) → keeps its initial. **Verified**: a rebuild that adds a retained global logs `restored 3/4` and the new variable starts at its initial while the other three continue.
- ⚠️ **`plc_retain_load()` is emitted LAST in `PLC_Init`**, i.e. after the injected `plc_state_init()` has written every initial value. The reverse order makes retain a silent no-op.
- `plc_retain_save()` packs, **compares against the last image written, and returns without touching the disk when nothing changed** — an idle machine costs an SD card nothing. Write is atomic: tmp + `fsync` + `rename` (⚠️ `MoveFileExA` on Windows — ISO `rename()` fails there when the destination exists, same trap as `host.c`'s `swap_result`).

### Who drives the cadence (1 s, `RETAIN_INTERVAL_MS`)
| Build | Flusher | Final flush on stop |
|---|---|---|
| Single binary, Linux (Build & Send) | `plc_retain_thread` (100 ms ticks so a stop is prompt) | `PLC_Cleanup`, reached because a **SIGTERM handler is installed — but ONLY for a project that has retained variables** (`retainEnabled`). Without it SIGTERM terminates outright and KronServer's Stop would skip `PLC_Cleanup` entirely; gating it keeps every other project's stop semantics untouched. |
| Single binary, Windows | the cooperative timer wheel in `main()` (plc.c includes `<pthread.h>` only under `__linux__`) | periodic flush only |
| Hot-swap (local sim, all 3 hosts) | `retain_thread` in `hotswaphost/host.c` — the LOADER-HOST owns it | `host.c` after the joins; `hotswap.go` `Stop` now sends SIGTERM and waits `hotSwapStopGrace` (1.5 s) before SIGKILL |

- ⚠️ **The logic module must NOT own the flusher thread.** It is `dlclose`d on every swap, so a thread running inside it would be pulled out from under itself — hence `plc_retain_save` is exported (non-static) and the host `dlsym`s it **optionally** (NULL when the project retains nothing) and **re-resolves it on every bind**.
- ⚠️ **`g_retain_mu` is what makes that safe**: the flusher runs OUTSIDE the scan barrier, so it can be executing the old module's `plc_retain_save` at the instant thread 0 `dlclose`s it. `do_swap` takes the same mutex across dlclose + re-resolve. Stress-verified: 12 back-to-back swaps straddling the flush window, all applied, no rollback, no crash.
- ⚠️ The retain thread is created **only when there are tasks** (both in the codegen and in `host.c`): with zero tasks nothing ever sets `plc_stop`, so an unconditional thread hangs `main()` on its join forever.

### Deliberate v1 limits — say these out loud rather than implying more
- **The snapshot is taken asynchronously**, not on a scan boundary, so a >word-sized value (LINT/LREAL on a 32-bit board) can in principle be sampled mid-update. Scan-synchronised capture needs per-task partitioning like the ring producer (§16).
- **Power-cut loss window is up to 1 s** (the flush interval) — that was the explicit design choice over per-change writes (flash wear) and stop-only writes (which do not survive a power cut at all).
- **A Build & Send does NOT reset retained values** — the name-keyed file survives the redeploy, so a changed program resumes with the old values. There is **no "clear retained values" button**: reset a single variable by force/PULSE-writing it from the variable table (the next flush persists it), or delete `retain.dat` from the deploy/build dir. A UI affordance is the obvious follow-up.
- **Marking retain does not change `PlcState`'s shape**, so it is not part of `layoutSignature` and never blocks a Hot Reload — but it only takes effect on the next swap/restart.

### The AI agent can set it (`retain: true`)
`add_variable`/`update_variable` take a **boolean `retain`**, not a free `class` string — retain is the only class the agent has any business steering, and a string would let a model set `Input` on a program local. ⚠️ `resolveRetainClass` refuses it outside a global / PROGRAM local with an error that routes to the real fix (retain the INSTANCE in the declaring program), and **that single guard is also what protects an FB's pin classes**: `retain:false` can never rewrite an `Input`/`Output`/`InOut` class to `Local` and silently drop a pin. `retain:false` only undoes an actual `Retain`, so a `Temp`/`Constant` marking survives it. The approval diff is prefixed **`RETAIN `** (`varLine`) because that diff is the human's only review surface for an agent edit, and `get_project_overview`/`read_pou` report `retain` so the model can see what already persists. Full contract in [`docs/PLC_AGENT.md`](docs/PLC_AGENT.md) §5.

### Verification
Driven end-to-end against the REAL transpiler + bundled clang (a global, a program local, a `CTU` instance retained; a plain global and a plain local as controls):
- **Linux single binary**: retained resumed (`g_retained` 140→160, `local_cnt` 20→30, `CTU.CV` 10→15), non-retained reset, SIGTERM handled gracefully, `retain.dat` 69 bytes.
- **Linux hot-swap pair** (`plc_host` + `logic_0.so`): identical results.
- **Windows** (`x86_64-w64-mingw32`, under wine64): `restored 4/4`, values continued across the restart.
- **Corrupt input**: garbage magic → rejected; truncated mid-record → restores the intact prefix (`restored 1/4`) and stops at the bound; empty → rejected. No crash in any case.
- **Agent surface** (real `applyToolCall`): retained global/program-local/FB-instance accepted, refused inside a function block and on an FB pin, `retain:false` leaves a `Temp` class alone, `RETAIN` shows in the diff, overview/read_pou report it. Closed loop verified — a project built ONLY through agent tool calls emits exactly the three marked variables into `__retain_tab`.
- aarch64 + armv7 cross-compiles clean; `go build` for linux/windows/darwin + `go vet` clean.
⚠️ **Not yet run on real hardware** — the acceptance test is a Build & Send to a board, a power cycle, and confirming the values come back.
